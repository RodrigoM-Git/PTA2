...
public class Car
{
	...
	/*
	 * Provides start of getDetails (shared with SilverService)
	 */
	public String getDetailsForCar() {
		StringBuilder sb = new StringBuilder();
	...
	/*
	 * Gets details of Current Bookings in a readable format.
	 */
	public String getCurrentBookings() {
		String cBookings = "CURRENT BOOKINGS: \n";
		...
		for(int i = 0; i < currentBookings.length; i++) {
			if(currentBookings[i] != null) {
				cBookings += "\t" + currentBookings[i].getDetails() + "\n";
			}
		}
		return cBookings;
	}
	...
	/*
	 * Gets details of Past Bookings in a readable format.
	 */
	public String getPastBookings() {
		String pBookings = "PAST BOOKINGS: \n";
		...
		for(int i = 0; i < pastBookings.length; i++) {
			if(pastBookings[i] != null) {
				pBookings += "\t" + pastBookings[i].getDetails() + "\n";
			}
		}
		return pBookings;
	}
	...
	String string = "";
		...
		string += toStringInitial();
		string += currentBookingsToString();
		string += pastBookingsToString();
		
		return string;
	}
	
	/*
	 * Gets current bookings in toString format
	 */
	...
	/*
	 * Gets past bookings in toString format
	 */
	...
	/*
	 * ALGORITHM - toString Initial method
	 * 	BEGIN
	 * 		GET regNo, make, model, driverName, passengerCapacity, and availability of car
	 * 		Print into toString format
	 * 		Return string
	 * 	END
	 */
	
	/*
	 * Provides start of toString (shared with SilverService)
	 */
	public String toStringInitial() {
		String initial = "";
		initial += regNo + ":" + make + ":" + model;
		if(driverName != null) {
			initial += ":" + driverName;
		}
		initial += ":" + passengerCapacity;
		if(bookingAvailable()) {
			initial += ":" + "YES";
		}else {
			initial += ":" + "NO";
	...
	private String completeBooking(int bookingIndex, double kilometers)
	{
		...
		double fee = kilometers * (getBookingFee() * getPercentOfTripFee());
		tripFee += fee;
		booking.completeBooking(kilometers, fee, getBookingFee());
	 ...
	 */
	private boolean dateIsValid(DateTime date) throws InvalidDate
	{
		//Exceptions for booking date
		if(!DateUtilities.dateIsNotInPast(date) || !DateUtilities.dateIsNotMoreThan7Days(date))
		{
			throw new InvalidDate("Error - Date is not valid...");
		}else {
